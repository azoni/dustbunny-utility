const values = {
	INFURA_KEY: ['9e30b32ca14a408c99ae890ac2c8e8dc', '9e30b32ca14a408c99ae890ac2c8e8dc', '1bac20c89d97488491bad84f22d7a15b', '231011f146004bd1927eaf77c8b69aac'],
	OWNER_ADDRESS: [
		{
			username: 'DustBunny',
			address: '0x13b451d77b87361d376ae211f640ed1a4491181d'
		}, 
		{
			username: 'BalloonAnimal',
			address: '0x4beac303c8fdf1f3cd34509b344067e86dcbc506'
		},
		],
	favorites: {
		'galacticapes': [9999],
		'theshiboshis': [9999],
		'kaiju-kingz': [3333],
		'cyberkongz': [4052],
		'cool-cats-nft': [9932],
		'world-of-women-nft': [9999],
		'cryptoadz-by-gremplin': [6969],
		'sappy-seals': [10000],
		'lazy-lions': [9999],
		'supducks': [9999],
		'metasaurs-nft': [9999],
		'jungle-freaks-by-trosley': [9999],
		'halloweenbears': [9999],
		'bears-deluxe': [6900],
		'ape-gang': [9999],
		'thehumanoids': [9999],
		'spaceapes': [2000],
		'capsulehouse': [9999],
		///
		'mutantcats': [9999],
		'winterbears': [9999],
		
		'robotos-official': [9999],
		'creature-world-collection': [9999],
		

		'peaceful-groupies': [9999],
		'onchainmonkey': [9999],
		'the-crypto-chicks': [9999],
		//Cheaper
		'mekaverse': [8593],
		'0n1-force': [7777],
		'sneaky-vampire-syndicate': [8880],
		'the-doge-pound': [9999],
		'pudgypenguins': [8887],
		'mutant-ape-yacht-club': [15671],
		'bored-ape-kennel-club': [9999],	
		'dapperdinosnft': [9911],
		'space-punks-club': [9999],
		'koala-intelligence-agency': [9999],
		'marscatsvoyage': [9999],
		'theclaylings': [4040],

		//tropical-turtles
	}
}
export default values

//dustbunny123@protonmail.com
//1bac20c89d97488491bad84f22d7a15b
///charlonuw
//9e30b32ca14a408c99ae890ac2c8e8dc
//dustbunny124@protonmail.com
//55b37dd4e48b49cb8c5f9e90445088a1
//dustuw
//231011f146004bd1927eaf77c8b69aac