...
let ourWallets = new Set(['0x18a73aaee970af9a797d944a7b982502e1e71556', '0x35c25ff925a61399a3b69e8c95c9487a1d82e7df']);
	...
	for(let o of orders){
		...
		try{
			...
			if(watchListCollection !== undefined){
				...
				asset = await redis_attach_range(event_type, asset, gasBaseFeeRightNow, GWEI.mul(3), 350_000, wallet.address, (txt) => { logger.print(txt) }, balance);
				//console.log('asset:')
				//console.log(asset)
...
function startGetBlockLoop() {
  provider.on('block', async (blockNumber) => {
    currBlockNo = blockNumber;
		...
		blockToFee[parseInt(blockNumber) + 1] = FlashbotsBundleProvider.getMaxBaseFeeInFutureBlock(block.baseFeePerGas, 1);
		blockToFee[parseInt(blockNumber) + 2] = FlashbotsBundleProvider.getMaxBaseFeeInFutureBlock(block.baseFeePerGas, 2);
		blockToFee[parseInt(blockNumber) + 3] = FlashbotsBundleProvider.getMaxBaseFeeInFutureBlock(block.baseFeePerGas, 3);
		delete blockToFee[parseInt(blockNumber) - 10];
