...
async function get_collection_bids(slug, exp, run_traits) {
  ...
  let start_index = 0
  let end = 3
  let start_loop = Math.floor(+new Date())
  // eslint-disable-next-line no-constant-condition
  while (true) {
    for (const token_array of token_ids.slice(start_index, end)) {
    ...
    const end_loop = Math.floor(+new Date())
    console.log(`runtime: ${((end_loop - start_loop) / 60000).toFixed(2)}`)
    if (end_loop - start_loop > 15 * 60000) {
      loop_counter += token_ids.slice(start_index, end).length * 30
      start_index += 3
      end += 3
